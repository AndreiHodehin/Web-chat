...
public class MainView extends VerticalLayout implements AppShellConfigurator {
    ...
    private void buildChat() {
        ...
        changeNameField.setPlaceholder("Enter another name");
        H3 nameH3 = new H3("Your name: " + user);
        chat.add(new H2("Vaadin chat"),
                nameH3,
                new HorizontalLayout(){{
                    add(                        changeNameField,
                        new Button("Change name") {{
                            addClickListener(click-> {
                                String oldName = user;
                                user = changeNameField.getValue();
                                changeNameField.clear();
                                storage.addRecordChangeName(user, oldName);
                            });
                        }}
                    );
    ...
    private String renderRow(Storage.ChatMessage message) {
        ...
        } else if (message.getMessage().startsWith("- new name of")) {
